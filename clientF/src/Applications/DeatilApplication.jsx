...
useEffect(()=>{
  ...
  const response=await axios.get(`https://internshalaclone.onrender.com/api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://internshalaclone.onrender.com/api/application/${id}`,{action})
