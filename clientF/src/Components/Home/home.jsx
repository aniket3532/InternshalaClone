...
function Home() {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://internshalaclone.onrender.com/api/internship`
        );
  ...
  return (
    <>
      <h1 className="text-center text-3xl font-bold z-10">
        {t("make_your_dream_career_a_reality")}
      </h1>
      <p className="text-center text-lg font-bold z-10">
        {t("trending_on_internarea")} 🔥
      </p>
                ...
                <div className="more flex justify-between mt-6">
                  ...
                  <Link to={`/detailInternship?q=${data._id}`}>
                    <span className="text-blue-500 mr-2">
                      {t("view_details")}<i class="bi bi-chevron-right"></i>
                    </span>
                  </Link>
                </div>
              </div>
...
``
      ...
      <div className="logins flex  h-32 mt-8">
        ...
        <div className="log flex">
          ...
          <a to="/register">
            <button className="btn6 "> {t("register")}</button>
          </a>
        </div>
      </div>
    </>
